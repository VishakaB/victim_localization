...
for i in range(1,rows):
    ...
    x_ids = str(int(df.loc[i][3])) # node id
    x1 = df.loc[i][4]
    x2 = df.loc[i][5]
...
def collect_data(j,emergarr,nodeids_gps,nodeids_radius):
    ...
    if (yy=="gNB"):
        ...
        nodeids_gps[yy] = (bench1,bench2)
        nodeids_loc[yy] = (bench1,bench2)
...
def forward_tracking(emergarr,nodearr,benchids_node,nodeids_gps,nodeids_radius,time):             
    ...
    i = 0
    for iter in nodearr:  #i is in the order of ids in nodeids
    ...
    for i in S:#each node
      astar = []
      benchids = node_benchids[itercount[i]].split(';')
      for kk in range(len(benchids)):#fpr each benchid 
        deltay=nodeids_loc[itercount[i]][1] - nodeids_loc[benchids[kk]][1]#kkth bench node y
        deltax=nodeids_loc[itercount[i]][0] - nodeids_loc[benchids[kk]][0]#kkth bench node y
        eudst = (deltax**2+deltay**2)**0.5
        astar=math.atan2(deltay,deltax)  
        if (benchids_aoa.get(itercount[i])==None):
          benchids_aoa[itercount[i]] = str(astar)
        else:
          lst0 =benchids_aoa[itercount[i]]
          lst1 = str(astar)
          benchids_aoa[itercount[i]] = (lst0+';'+lst1)
        if (benchids_dst.get(itercount[i])==None):
          benchids_dst[itercount[i]] = str(eudst)
        else:
          lst0 =benchids_dst[itercount[i]]
          lst1 = str(eudst)
          benchids_dst[itercount[i]] = (lst0+';'+lst1)
    ...
    for i in S:#each node
      ...
      benchids = node_benchids[itercount[i]].split(';')
      benchdsts = benchids_dst[itercount[i]].split(';')
      #for each node, for the length of benchmarks=  nb distance constraints
      for kk in range(len(benchids)):
        d[kk] = float(benchdsts[kk])#distance value set in benchid order #true distance
        bxy = nodeids_loc[benchids[kk]]
        ...
        #aoa constraint
        benchaoas = benchids_aoa[itercount[i]].split(';')
        op=0.1#aoa error
        a[kk] = float(benchaoas[kk])#in benchid order #true angle
