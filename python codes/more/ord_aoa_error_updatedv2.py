...
# =================== 0. Import data ====================
df = pd. read_excel (r'15kv32.xlsx', sheet_name='Input')
rows = df.shape[0]
columns = df.shape[1]
random.seed(100)
...
def theta_limits(theta_truth, max_error_angle):
    if (theta_truth<0):
        theta_truth=theta_truth+2*math.pi # form anticlockwise angle 

    amin=theta_truth-(math.pi/16)/2#theta error opening 
    amax=theta_truth+(math.pi/16)/2

    #keep interval theta in the same quadrant as the theta truth
    if amin<0:
      amin=2*math.pi+amin
    if amax>2*math.pi :
      amax=amax-2*math.pi
...
def collect_data(j,emergarr,all_x_gps):
    ...
    if (outdoor_los==True):
        ...
        e_toa = ydist*Interval(random.randint(-10000,-1000),random.randint(1000,10000))
    ...
    theta_est = theta_limits(theta1,math.pi/3)

    if (no_aoa == False):
        #theta_est =[Interval(0,2*math.pi)]#estimated theta of bench nodes
    #else:
        theta_est = [Interval(amin,amax)]#estimated theta of bench nodes 
        #if theta1!=float(6.28) and (float(theta_truth[0])!=6.28) and (theta1)<3.14:#incoverage benchark    
    ...
    if (toa_availability==True):
        ...
        #theta_est = [Interval(amin,amax),Interval(amin,amax)]#estimated theta of bench nodes 
        #theta_est = [Interval(0,2*math.pi),Interval(0,2*math.pi)]
...
def forward_tracking(node_ids,node_benchids,node_benchtruth,node_theta,node_y,node_heading,time,emergarr,all_x_gps,all_gps_radius,all_x_gpstheta):
    for iter in node_ids:#for each node recorded till now      
            ...
            v_obs = DataLoader.generate_static_observations(node_xtruth[iter],b, False)
            #print(v_obs)
            # Adding uncertainties on the measurements
            for hh in v_obs: # for each observation:
              hh[0].inflate(0.0) # range
            ...
            #print(v_obs2)
            for gg in v_obs2: # for each observation:
            ...
            #print('**',radius_gps)
            if (radius_gps.is_empty() or b[0].is_empty() or v_obs[0].is_empty()):
                yyy= 0
                ...
                for i in range (0,len(v_obs)): # for each range and bearing measurement
                      ...
                      #cn.add(ctc_dist,[b[i],p, v_obs[i][0]])
                      deltax = b[i][0]-p[0]
                      deltay = b[i][1]-p[1]
                      ratio = deltay/deltax
                      ...
                      if (node_incoverage.get(iter)!=None):
                        a = theta_limits(v_obs[i][1].mid(),math.pi/16)
                        theta_est =  IntervalVector(a)
                        cn.add(ctc_plus, [theta_est, heading, alpha])
                        cn.add(ctc_polar, [d, v_obs[i][0], alpha])
                        cn.add(ctc_polarx,[b[i][0],p[0],v_obs[i][0],alpha])#x coordinates difference
                        cn.add(ctc_polary,[b[i][1],p[1],v_obs[i][0],alpha])#y coordinates difference     
                #cn.add(ctc.dist, [p, gps, v_obs2[0][0]])
                #cn.add(ctc_plus, [v_obs2[0][1], gpsheading, alpha])
                r = cn.create_dom(IntervalVector(2)) 
                ...
                #print(p)
                #print([p[0].mid(),p[1].mid()],[x1,x2],mean_square_error([x1,x2],[p[0].mid(),p[1].mid()]))
                #cn.contract(True)
...
ctc_angle = CtcFunction(Function("a","b","c","atan2(b,a)-c"))
#ctc_atan2 = CtcFunction(Function("a", "b", "c", "atan2(a-b)=c"))
ctc_polar = CtcPolar()
