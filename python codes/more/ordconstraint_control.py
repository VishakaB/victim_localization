...
bench_heading ,btruth, ntruth={},{}, {}

def error_gps(outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        ...
        e_g = Interval(random.randint(-3500,0),random.randint(0,3500))
    return e_g
...
for i in range(1,rows):
    ...
    e_g= error_gps(outdoor_los,outdoor_nlos,indoor_nlos)
    g1 = Interval(xgps_all[x_ids])#gps coordinates
...
def error_range(range,outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        ...
        e_y = range*Interval(random.randint(-3000,-2500),random.randint(2500,3000))
    return e_y

def error_range_toa(range,outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        e_y = 0.01*range*Interval(random.randint(-150,-30),random.randint(30,150))
    elif (outdoor_nlos==True):
        e_y = 0.01*range*Interval(random.randint(-1250,-150),random.randint(150,1250))
    elif (indoor_nlos==True):
        e_y = 0.01*range*Interval(random.randint(-3000,-2500),random.randint(2500,3000))
    return e_y
...
def collect_data(j,emergarr,all_x_gps):
    ...
    #e_bs =  error_gps(outdoor_los,outdoor_nlos,indoor_nlos)
    e_bs = Interval(random.randint(-100,0),random.randint(0,100))
    ...
    if (yy=="gNB"):
        ...
        bench1 = float(benchmark1_truth[0])
        bench2 = float(benchmark1_truth[1])
        #e_b = Interval(random.randint(-100,0),random.randint(0,100))
        e_b=error_gps(outdoor_los,outdoor_nlos,indoor_nlos)
        b1 = Interval(bench1+e_b)
        b2 = Interval(bench2+e_b)#estimated locations of bench nodes 
            ...
            if (b[0].is_empty()):
                ...
                if (time_complexity.get(time) ==None):
                    ...
                    for i in range (0,len(b)): # for each range and bearing measurement
                        q = cn.create_dom(IntervalVector(2))     
                        ...
                        if (node_incoverage.get(iter)==None):#out of coverage                         
                            for rg in v_obs: # for observation type:range#shifting the actual range to a point within interval of error
                              k = random.choice([-1, 1])*random.randrange(2500,3000)
                            ...
                            for hh in v_obs: # for each observation:
                              e_y = 0.01*hh[0]*Interval(random.randint(-3000,-2500),random.randint(2500,3000))
                              hh[0]= hh[0]+e_y#range
                              hh[1].inflate(0.0) # bearing 
                            cn.add(ctc.dist, [q, b[i], v_obs[0][0]])# Distance constraint: relation between the state at t_i and the ith beacon position
                        if (node_incoverage.get(iter)!=None):#incoverage                        
                            ...
                            for i in range (0,len(v_obs_toa)):
                                  ...
                                  cn.add(ctc.dist, [q, b[i], v_obs_toa[i][0]])#toa based-range constraint
                    cn.add(ctc_circle,[q[0],q[1],gps[0],gps[1],gps_radius])    
                    contraction_dt = cn.contract()
                    if (q[0].is_empty()):
                        print('iter',iter)
                        #print('b',b)
                    ...
                    for i in range (0,len(b)): # for each range and bearing measurement
                          ...
                          if (node_incoverage.get(iter)!=None):#incoverage                        
                              ...
                              heading = btruth[yy][2]
                              bangle = Interval(math.atan2(X[1]-Y[1],X[0]-Y[0]))
                              #-heading
                              a = theta_limits(bangle.mid(),outdoor_los,outdoor_nlos,indoor_nlos,btruth[yy][2])
                              theta_est =  IntervalVector(a)                          
                              amin = theta_est[0][0]
                              amax = theta_est[0][1]
                              if amax<=3.14 and amin>-3.14:
                                alpha=  IntervalVector([Interval(amin,amax)])#in radians
                                cn.add(ctc_taninverse, [p,b[i], alpha])
                                #cn.add(ctc_plus, [theta_est, heading, alpha])#angle constraint#heading wrt bench
                                #cn.add(ctc_minus, [p,b[i], d])
                                #cn.add(ctc_polar, [d,v_obs[0][0], alpha])
                              elif amin<=-3.14159 :
                    ...
                    if p.contains(node_xtruth[iter]):
                        ...
                        tgop=0
                    #err mean
                    if (p[0].is_empty() and q[0].is_empty() ):
                        print('iter',iter)
                        print('b',b)
                        print('xtruth',node_xtruth[iter])
                        print('p',p)
                        print('yea',gps,id)
                    elif (p[0].is_empty()):#q is non empty
                        p = q 
...
ctc_taninverse =CtcFunction(Function("px", "py", "bx","by","t","atan2(py-by,px-bx)-t"))
ctc_sinlimits = CtcFunction(Function("px", "py", "bx","by","t","(py-by)/((px-bx)^2+(py-by)^2)-t"))
ctc_polar = CtcPolar()
