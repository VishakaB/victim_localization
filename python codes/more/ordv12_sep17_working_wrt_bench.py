...
# =================== 0. Import data =========================================================================================================================================================
df = pd. read_excel (r'15kv37.xlsx', sheet_name='Input')
rows = df.shape[0]
columns = df.shape[1]
random.seed(100)
outdoor_los=False
...
def error_range(range,outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        ...
        e_y = range*Interval(random.randint(-3000,-1000),random.randint(1000,3000))
    return e_y
...
def error_range_toa(range,outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        ...
        e_y = range*Interval(random.randint(-2500,-2000),random.randint(2000,2500))
    return e_y
...
def collect_data(j,emergarr,all_x_gps):
    ...
    e_bs = Interval(random.randint(-3500,-1200),random.randint(1200,3500))
                ...
                for i in range (0,len(b)): # for each range and bearing measurement
                      ...
                      if (node_incoverage.get(iter)==None):#out of coverage                         
                          for rg in v_obs: # for observation type:range#shifting the actual range to a point within interval of error
                            k = random.choice([-1, 1])*random.randrange(1000,3000)
                          ...
                          for hh in v_obs: # for each observation:
                            e_y = 1*hh[0]*Interval(random.randint(-3000,-1000),random.randint(1000,3000))
                            hh[0]= hh[0]+e_y#range
