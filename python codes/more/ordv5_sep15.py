...
def collect_data(j,emergarr,all_x_gps):
    ...
    if (outdoor_los==True):
        ...
        e_g = Interval(random.randint(-1000,-100),random.randint(100,1000))
    ####################################################################################################
    bench_heading[yy] = Interval(math.atan2(float(benchmark1_truth[1]),float(benchmark1_truth[0]))).inflate(0.01)
    ####################################################################################################
    toa_availability= False
    if (yy=="gNB"):
            ...
            if (v_obs[0].is_empty()):
                ...
                for i in range (0,len(v_obs)): # for each range and bearing measurement
                      ...
                      if (node_incoverage.get(iter)!=None):
                          ...
                          #print('theta_est',theta_est,heading)
                          #cn.add(ctc_plus, [heading,theta_est, balpha])
                          cn.add(ctc_polar, [d, v_obs[i][0], balpha])
                          cn.add(ctc_plus, [balpha,heading, theta_est])
                          #cn.add(ctc.polar, [d, y1, balpha])
                          #cn.add(ctc_polarx,[p[0],b[i][0],v_obs[i][0],theta_est])#x coordinates difference
                          #cn.add(ctc_polary,[p[1],b[i][1],v_obs[i][0],theta_est])#y coordinates difference   
