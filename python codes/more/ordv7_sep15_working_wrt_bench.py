            ...
            if (v_obs[0].is_empty()):
                ...
                for i in range (0,len(v_obs)): # for each range and bearing measurement
                      ...
                      if (node_incoverage.get(iter)!=None):
                          #print(benchids)
                          yy = benchids[i]
                          ntruth = [IntervalVector([node_xtruth[iter][0],node_xtruth[iter][1]])]
                          v_bangle = DataLoader.generate_static_observations(btruth[yy],ntruth,False)
                          a = theta_limits(v_bangle[0][1].mid(),outdoor_los,outdoor_nlos,indoor_nlos)
                          theta_est =  IntervalVector(a)
                          #cn.add(ctc_plus, [v_bangle[0][1], btruth[yy][2], alpha])#heading wrt bench
                          cn.add(ctc_polar, [d, v_obs[i][0], alpha])
                          cn.add(ctc_minus, [p,b[i], d]) #wrt bench
