...
def error_range_toa(range,outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        e_y = 0.1*range*Interval(random.randint(-10,-1),random.randint(1,10))
    elif (outdoor_nlos==True):
            ...
            if (b[0].is_empty() or gps[0].is_empty()):#outliers will not be considered
                ...
                for i in range (0,len(b)): # for each range and bearing measurement
                      ...
                      for rg in v_obs: # for observation type:range#shifting the actual range to a point within interval of error
                        k = random.choice([-1, 1])*random.randrange(500,1000)
                      ...
                      for hh in v_obs: # for each observation:
                        e_y = 0.1*hh[0]*Interval(random.randint(-1000,-500),random.randint(500,1000))################################################################################
                        hh[0]= hh[0]+e_y#range
                        hh[1].inflate(0.0) # bearing 
                        if (b[i].is_empty()):#outliers removed
                          uuuy=0
                        else:
                ...
                cn.add(ctc.dist,[p,gps,gps_radius])   
                #print(iter)
                #print(cn)      
