...
# =================== 0. Import data =========================================================================================================================================================
df = pd. read_excel (r'15kv34.xlsx', sheet_name='priority')
rows = df.shape[0]
columns = df.shape[1]
random.seed(100)
outdoor_los=False
...
bench_heading ,btruth, ntruth,ntruthaoa,id_inside,id_box_area,id_errorlist={},{}, {},{},{},{},{}
...
for i in range(1,rows):
    ...
    if xgps_all.get(x_ids) == None:
        ...
        gps_radius = 1954.737/(997.368**(float(df.loc[i][11])))#worst : 1997, best : 2m
        #print(gps_radius)
...
def error_range_toa(range,outdoor_los,outdoor_nlos,indoor_nlos):
    if (outdoor_los==True):
        e_y = Interval(random.randint(-150,-30),random.randint(30,150))
    elif (outdoor_nlos==True):
        e_y = Interval(random.randint(-1250,-150),random.randint(150,1250))
    elif (indoor_nlos==True):
        e_y = Interval(random.randint(-200,0),random.randint(0,200))
    return e_y
            ...
            if (b[0].is_empty()):
                ...
                for i in range (0,len(b)): # for each range and bearing measurement
                      ...
                      for rg in v_obs: # for observation type:range#shifting the actual range to a point within interval of error
                        k = random.choice([-1, 1])*random.randrange(0,2000)
                ...
                if (p[0].is_empty()):
                    ...
                    if iter in emergarr:#only emerg node ids
                          ...
                          boxarea = loc_area(p[0][0],p[1][0],p[0][1],p[1][1])
                          if (id_box_area.get(iter)== None):#nodeid, benchtruth location 
                              id_box_area[iter] = str(boxarea)+" , "+str(len(node_benchtruth[iter]))
                          else:
                              lst0 = id_box_area[iter]
                              lst = str(boxarea)+" , "+str(len(node_benchtruth[iter]))
                              id_box_area[iter] = (str(lst0)+" ; "+str(lst))
                          if (id_errorlist.get(iter)== None):#nodeid, benchtruth location 
                              id_errorlist[iter] = str(er)+" , "+str(len(node_benchtruth[iter]))
                          else:
                              lst0 = id_errorlist[iter]
                              lst = str(er)+" , "+str(len(node_benchtruth[iter]))
                              id_errorlist[iter] = (str(lst0)+" ; "+str(lst))
...
while thirty_min_lb <= thirty_min_ub:#while now is below the upper bound of time domain
    ...
    if float(df.loc[jj][13])< next_tub_stop and float(df.loc[jj][13])<= t_ub:  
         ...
         if (float(df.loc[jj][13])>thirty_min_ub):           
            ...
            node_benchids = {}#data refresh#time window
            node_benchtruth = {}
            node_y = {}
            node_theta = {}
            node_ids = {}
         ...
         node_benchids = {}#data refresh#do not refresh sol ids#seq
         node_benchtruth = {}
         node_y = {}
         node_theta = {}
         node_ids = {}
         nodearr = []
         #sol_ids={}#comment this for seq
...
vy = 1
sheet11 = book.add_sheet("Sheet 11")    
for n in range(0,len(id_inside)):
    vy = vy+1#row number
    gg = id_time_arr[n]
    x = gg.split(';')
    h=id_inside[gg] 
    sheet11.write(vy, 0, gg)
    sheet11.write(vy, 1,str(h))
    sheet11.write(vy, 2, str(node_xtruth[float(x[0])]))
 
bc = 1
sheet12 = book.add_sheet("Sheet 12")    
for n in range(0,len(id_box_area)):
    bc= bc+1#row number
    gg = id_arr[n]
    h  = id_box_area[int(gg)]
    sheet12.write(bc, 0, int(gg))
    sheet12.write(bc, 1, h)

ab = 1
sheet13 = book.add_sheet("Sheet 13")    
for n in range(0,len(id_errorlist)):
    ab= ab+1#row number
    gg = id_arr[n]
    h  = id_errorlist[int(gg)]
    sheet13.write(ab, 0, int(gg))
    sheet13.write(ab, 1, h)
    
book.save("allkv33 seq_results.xls")