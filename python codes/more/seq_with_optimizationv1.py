...
import itertools
# =================== 0. Import data ====================
df = pd. read_excel (r'15kv35.xlsx', sheet_name='priority')
rows = df.shape[0]
columns = df.shape[1]
random.seed(100)
outdoor_los=True
...
for i in range(1,rows):
    ...
    #all_gps_radius[x_ids]= Interval(eudist)
...
def LinearProgrammingExample():
    ...
    #csp problem run#solution_okay==True
    ...
    solver.Maximize(x1*1+x2*2+x3*3+x4*4)
...
def forward_tracking(node_ids,node_benchids,node_benchtruth,node_heading,time,emergarr,all_x_gps,all_gps_radius,all_x_gpstheta):
    for iter in node_ids:#for each node recorded till now      
            ...
            if (b[0].is_empty() or gps[0].is_empty()):#outliers will not be considered
                ...
                x3 = solver.BoolVar('x3')
                x4 = solver.BoolVar('x4')
                
                
                lst = list(itertools.product([0, 1], repeat=4))
                #print(lst)
                while (solution_okay==False):
                    for u in range(len(lst)-1,1,-1):
                      x = lst[u]
                      #print('*',x)
                      ...
                      for i in range (0,len(b)): # for each range and bearing measurement
                            ...
                            for hh in v_obs: # for each observation:
                              ...
                              #print('x1',x1)
                              cn.add(ctc.dist, [x[2]*p, x[2]*b[i], x[2]*v_obs[0][0]])# Distance constraint: relation between the state at t_i and the ith beacon position
                            if (node_incoverage.get(iter)!=None):#incoverage                        
                                ...
                                for i in range (0,len(v_obs_toa)):
                                      ...
                                      cn.add(ctc.dist, [x[0]*p, x[0]*b[i], x[0]*v_obs_toa[i][0]])#toa based-range constraint
                                ...
                                if amin>-3.14159 and amax<=3.14159:
                                  ...
                                  cn.add(ctc_taninverse, [x[3]*p,x[3]*b[i],x[3]*heading, x3*theta_est])
                                elif amin<=-3.14159 :
                                  sin_lb = math.sin(amax)
                                  sin_ub = math.sin(6.28318+amin)
                                  sin_limits = IntervalVector([Interval(sin_lb,sin_ub)])
                                  cn.add(ctc_sinlimits, [x[3]*p,x[3]*b[i], x[3]*sin_limits])
                                elif amax>3.14159 :
                      ...
                      cn.add(ctc.dist,[x[1]*p,x[1]*gps,x[1]*gps_radius])       
