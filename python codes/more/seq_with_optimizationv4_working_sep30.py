...
# =================== 0. Import data ====================
df = pd. read_excel (r'15kv31.xlsx', sheet_name='priority')
rows = df.shape[0]
columns = df.shape[1]
random.seed(100)
outdoor_los=False
...
for i in range(1,rows):
    ...
    if xgps_all.get(x_ids) == None:
        ...
        angle = 0.01*random.randint(0,628)
        #gps_radius=0
...
def forward_tracking(node_ids,node_benchids,node_benchtruth,node_heading,time,emergarr,all_x_gps,all_gps_radius,all_x_gpstheta):
    for iter in node_ids:#for each node recorded till now      
            ...
            #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
            benchids = node_benchids[str(iter)].split(';')
            if (b[0].is_empty() or gps[0].is_empty()):#outliers will not be considered
                yyy= 0
                print('vobs empty',iter)
            else:
                ...
                emptytimes=0
                while (solution_okay==False):#outlier processing
                    for u in range(len(lst)-1,0,-1):
                        x = lst[u]
                        #print(x)
                        cn = ContractorNetwork() 
                        ...
                        for i in range (0,len(b)): # for each range and bearing measurement
                                ...
                                for hh in v_obs: # for each observation:
                                  ...
                                  #print('x1',x1)
                                  #print('1',x[2])
                                  cn.add(ctc.dist, [p, x[2]*b[i], x[2]*v_obs[0][0]])# Distance constraint: relation between the state at t_i and the ith beacon position
                                if (node_incoverage.get(iter)!=None):#incoverage                        
                                    ...
                                    for i in range (0,len(v_obs_toa)):
                                          ...
                                          #print('2',x[0])
                                          cn.add(ctc.dist, [p, x[0]*b[i], x[0]*v_obs_toa[i][0]])#toa based-range constraint
                                    ...
                                    if amin>-3.14159 and amax<=3.14159:
                                      ...
                                      cn.add(ctc_taninverse, [p,x[3]*b[i],x[3]*heading, x[3]*theta_est])
                                      #print('3',x[3])
                                    elif amin<=-3.14159 :
                                      sin_lb = math.sin(amax)
                                      sin_ub = math.sin(6.28318+amin)
                                      sin_limits = IntervalVector([Interval(sin_lb,sin_ub)])
                                      cn.add(ctc_sinlimits, [p,x[3]*b[i], x[3]*sin_limits])
                                      #print('4',x[3])
                                    elif amax>3.14159 :
                        ...
                        cn.add(ctc.dist,[p,x[1]*gps,x[1]*gps_radius]) 
                            #print('9',x[1])
                        contraction_dt = cn.contract()#csp solving time set to 2 sec
                        #print('10')
                        if (p[0].is_empty()):
                            print('!!!!!!!!!!!!!!!!!!empty',iter,emptytimes)
                            print(x)
                            emptytimes+=1
                            if (x[0]==0 and x[1]==0 and x[2]==0 and x[3]==0 or emptytimes >15):
                                print('not solved',x,iter)
                                #emptytimes=0#reset variable
                                if (emptytimes >15):
                                    solution_okay = True
                                break
                            solution_okay = False
                ...
                if p[0].is_empty()==False and iter in emergarr:#only emerg node ids
                      sol.append(p)
                      sol_ids[iter] =p
...
book.save("seq only cellular optimized.xls")
